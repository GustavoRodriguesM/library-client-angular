...
export class CategoryAllInOneComponent implements OnInit {
  ...
  save(){
    this.categoryService.save(this.category).subscribe(()=>{this.findAll()}, err => console.log(err));
  }

  update(){
    this.categoryService.update(this.category).subscribe(()=>{this.findAll(), this.reset()}, err => console.log(err));
  }
